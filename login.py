...
#resp=request.urlopen(req)
    ...
    img_req=request.Request(img_url,headers=headers)
    img_resp=request.urlopen(img_req)
    img_resp=opener.open(img_url)
    img_data=img_resp.read()
    fd=open('checkcode.jpg','wb')
    fd.write(img_data)
    fd.close()
    form['secretCode']=input('请输入验证码')
    post_data=parse.urlencode(form).encode('utf-8')	    
    r=opener.open(url,post_data)
    result=r.read().decode('utf-8')
    llist=re.findall(r'<span>当前用户.+?\(([0-9]+)\)',result)
#print(number)
    if llist==[]:
        username+=1
        password+=1
...
#print(type(page))
    ...
    score_req=request.Request(score_url,headers=headers)
...
#print(file_name==username)
    ...
    p_req=request.Request(photo_url,headers=headers)
